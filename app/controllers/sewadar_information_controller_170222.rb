...
class SewadarInformationController < ApplicationController
  ...
  def generate_sewadar_series
        cattype      = params[:catname] != nil && params[:catname] != '' ? params[:catname].to_s.strip.split("-") : ''
        mycatttype   = params[:catname] != nil && params[:catname] != '' ? params[:catname].to_s.strip : ''
        
        if mycatttype == 'SEWADAR TEMPORARY' || mycatttype == 'SEWADAR REGULAR' || mycatttype == 'SEWADAR ASSOCIATE A' || mycatttype == 'SEWADAR ASSOCIATE B'
          catcodes     = 'SDP'
          prefixobj    = 'SD'
          @recCodes    = MstSewadar.select("sw_sewcode").where(["sw_compcode = ? AND sw_catcode NOT IN('VIO','VIV','DWD','VIF') AND sw_sewcode<>''", @compCodes]).order("sw_sewcode DESC").first
        else
          catcodes     = cattype[0] ? cattype[0] : ''
          prefixobj    = catcodes
  ...
  def sewadar_params
        ...
        catscode                    = params[:sw_catgeory] !=nil && params[:sw_catgeory] !='' ?  params[:sw_catgeory].to_s.strip.split("-") : ''
        params[:sw_catcode]         = catscode[0] ? catscode[0] : ''
        ...
        if sewimages.to_s != "personnel_boy.png"
            ...
            if params[:sw_image] == nil ||  params[:sw_image] == ''
                if params[:currentimg] != nil && params[:currentimg] !=''
                  myimages = @profileimg =  params[:currentimg]
                end
            end
        end
  ...
  def get_sewadar_listing
    ...
    if session[:autherizedUserType] && session[:autherizedUserType].to_s != 'adm'
     ...
     if myflagsjs || mytflags
       listobj =  MstSewadar.select(isselects).joins(jons).where(iswhere).paginate(:page =>pages,:per_page => 20).order("sw_catcode ASC,sw_sewcode ASC")
     else
       listobj =  MstSewadar.where(iswhere).paginate(:page =>pages,:per_page => 20).order("sw_catcode ASC,sw_sewcode ASC")
     end
...
private
  ...
  def process_office_params
    ######## ADD PRESONAL INFORMATION ###
       ...
       params.permit(:so_compcode,:so_subdepartment,:so_location,:so_sublocation,:so_zone,:so_branch,:so_oldsewdar,:so_biomatriccard,:so_sewcode,:so_desigcode,:so_deprtcode,:so_respcode,:so_qualifcode,:so_joiningdate,:so_regularizationdate,:so_superannuationdate,:so_leavingdate,:so_fullfinaldate,:so_leavingreason)
  ...
  def update_maiantaince
      ...
      svuobj =  MstSewadarOfficeInfo.where("so_compcode = ? AND so_sewcode = ?",compcode,sewacode).first
       if svuobj
		 ...
		 save_single_yes_no_list(sewacode,params[:sk_electricconsumption],params[:sk_accomodations],params[:sw_meterno],params[:electexmption],params[:accomoexmption])
  ...
  def office_list_details(sewcode,so_blessedbrahma,sp_gyandate1)
        ...
        if svuobj          
           svuobj.update(:so_blessedbrahma=>so_blessedbrahma,:sp_gyandate=>sp_gyandate)
            ...
            svbsobj = MstSewadarOfficeInfo.new(:so_compcode=>compcode,:so_blessedbrahma=>so_blessedbrahma,:sp_gyandate=>sp_gyandate,:so_location=>0,:so_sublocation=>0,:so_sewcode=>sewcode,:so_joiningdate=>0,:so_regularizationdate=>0,:so_superannuationdate=>0,:so_leavingdate=>0,:so_fullfinaldate=>0,:so_settmentdate=>0)
            svbsobj.save
       end
  end
  ####### END MAINTAINANCE ######
   ...
   def process_family_detail
      ...
      if skf_attachment !=nil && skf_attachment !=''
       ...
       if skf_dependent !=nil && skf_dependent !=''
           process_save_family(compcodes,sewcode,skf_dependent,skf_relation,skf_datebirth,skf_address,skf_occupation,skf_nominee,skf_percentage,skf_nomineebank,skf_optedpolicy,skf_pannumber,skf_attachment,skf_gender,footerid)
           processcount +=1
       end
   end
   
   private
   def process_save_family(skf_compcode,skf_sewcode,skf_dependent,skf_relation,skf_datebirth,skf_address,skf_occupation,skf_nominee,skf_percentage,skf_nomineebank,skf_optedpolicy,skf_pannumber,skf_attachment,skf_gender,footerid)
            svuobj = MstSewdarKycFamilyDetail.where("skf_compcode =?  AND id = ?",skf_compcode,footerid).first
            if svuobj
                 svuobj.update(:skf_dependent=>skf_dependent,:skf_gender=>skf_gender,:skf_pannumber=>skf_pannumber,:skf_attachment=>skf_attachment,:skf_relation=>skf_relation,:skf_datebirth=>skf_datebirth,:skf_address=>skf_address,:skf_occupation=>skf_occupation,:skf_nominee=>skf_nominee,:skf_percentage=>skf_percentage,:skf_nomineebank=>skf_nomineebank,:skf_optedpolicy=>skf_optedpolicy)
                 ### SAVE EXECUTE
            else                
                svupobj = MstSewdarKycFamilyDetail.new(:skf_compcode=>skf_compcode,:skf_sewcode=>skf_sewcode,:skf_gender=>skf_gender,:skf_dependent=>skf_dependent,:skf_pannumber=>skf_pannumber,:skf_attachment=>skf_attachment,:skf_relation=>skf_relation,:skf_datebirth=>skf_datebirth,:skf_address=>skf_address,:skf_occupation=>skf_occupation,:skf_nominee=>skf_nominee,:skf_percentage=>skf_percentage,:skf_nomineebank=>skf_nomineebank,:skf_optedpolicy=>skf_optedpolicy)
                if svupobj.save
                  ### SAVE EXECUTE
                end
          end
  ...
  ### END PROCESS KYC ##
   ...
   private
   def save_single_yes_no_list(sewcode,electricconsumption,accomodation,electricmeter,electricexmep,accomodexempt)
		compcodes     =  session[:loggedUserCompCode]
    ...
    accomodexempt = accomodexempt !=nil && accomodexempt !='' ? accomodexempt : 'N'
		...
		if sewdadobj
		   sewdadobj.update(:sw_isaccommodation=>accomodation,:sw_iselectricconsump=>electricconsumption,:sw_meterno=>electricmeter,:sw_electricexemption=>electricexmep,:sw_accomodexemption=>accomodexempt);
		end
   end
