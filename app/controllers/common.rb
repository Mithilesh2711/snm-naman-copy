  ...
  module Common 
   ...
   def formatted_times(times)
    ...
    ########## HR MONTHLY SALAY CALCULATION #########
      ...
      private
      def get_sewadar_monthly_list(compcode,sewacode)
        isselect ="pm_paidleave,pm_workingday,pm_wo,pm_hl,pm_payyear,pm_monthday,pm_areaprvmonths,pm_areaprvyears,pm_paydays,pm_absent"
        mobjs   = TrnPayMonthly.select(isselect).where("pm_compcode =? AND pm_sewacode =?",compcode,sewacode).first
        return mobjs
      end
      private
