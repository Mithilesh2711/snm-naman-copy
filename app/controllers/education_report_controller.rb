class EducationReportController < ApplicationController
    ...
    helper_method :get_sewa_all_department,:get_sewa_all_qualification,:get_sewa_all_rolesresp,:get_sewa_all_designation,:get_first_my_sewadar,:get_banking_detail_kyc_bankdetail
    helper_method :get_sewadar_dependent_listed,:get_sewadar_kyc_family,:get_sewadar_kyc_qualification,:get_dependent_education_detail,:get_common_unversity_firstrecord
    helper_method :get_vouchers_detail
    def index
...
private
  ...
  def get_dependent_education_detail(compcode,dependent)
    chkdsobj      = TrnApplyEducationAid.where("aea_compcode = ? AND aea_dependent = ? AND aea_status<>'R' AND aea_status<>'C' AND aea_voucherno<>''",compcode,dependent).first #AND YEAR(aea_requestdate)=YEAR(NOW()) 

  end
  private
  def get_vouchers_detail(compcode,vouvherno)
    chkdsobj    = TrnVoucherDetail.where("vd_compcode = ? AND vd_voucherno = ?",compcode,vouvherno).first
  end
