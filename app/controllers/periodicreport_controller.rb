 ...
 if session[:requestuser_loggedintp] && session[:requestuser_loggedintp].to_s == 'swd' || session[:requestuser_loggedintp].to_s == 'stf'
  ...
  def print_new_sewadar_salary
    ...
    if fromdate !=nil && fromdate !=''
      fromdate = fromdate.to_s.split("-")
      fromdate = fromdate[0].to_s+"-"+fromdate[1].to_s+"-01"
    end

    iswhere   = "pm_compcode ='#{@compCodes}' AND pm_sewacode='#{pmssewacode}'" 
    iswhere   += " AND  STR_TO_DATE(CONCAT(pm_payyear,'-',pm_paymonth,'-','01'),'%Y-%m-%d')>='#{fromdate}' AND  STR_TO_DATE(CONCAT(pm_payyear,'-',pm_paymonth,'-','30'),'%Y-%m-%d')<='#{updated}' "
    ...
    pmsobj = TrnPayMonthly.select(isselect).joins(jons).where(iswhere).order("pm_payyear ASC ,pm_paymonth ASC") #"LPAD(lower(pm_payyear), 10,0) ASC,LPAD(lower(pm_paymonth), 30,0) ASC"
