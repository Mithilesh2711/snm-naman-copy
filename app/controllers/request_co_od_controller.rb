...
def request_co_od_new_refresh
    ...
    session[:req_myuseaccord]         = nil
    session[:req_myusestring]         = nil
    session[:req_myleave_type]        = nil
    session[:req_mysearch_fromdated]  = nil
    session[:req_mysearch_uptodated]  = nil
   redirect_to "#{root_url}"+"request_co_od"
end
...
def leave_details_primary(empcode,years)
   isleave = TrnLeaveBalance.where("lb_compcode = ? AND lb_empcode = ?  ",@compCodes,empcode)
   return isleave
end
private
...
def get_apply_leaves
      ...
      if params[:page].to_i >0
       ...
       leave_type          = params[:leave_type] != nil && params[:leave_type] !='' ? params[:leave_type] : session[:req_myleave_type] 
       search_fromdated    = params[:search_fromdated] != nil && params[:search_fromdated] !='' ? params[:search_fromdated] : session[:req_mysearch_fromdated] 
       search_uptodated    = params[:search_uptodated] != nil && params[:search_uptodated] !='' ? params[:search_uptodated] : session[:req_mysearch_uptodated] 
     ...
     if leave_type !=nil && leave_type !=''
         session[:req_myleave_type]  = leave_type
         @leave_type = leave_type
         if leave_type == 'P'
            iswhere += " AND ls_status IN('','N','P') "
         elsif leave_type == 'all'
            ## excute while its required. 
         else
     ...
     if search_fromdated !=nil && search_fromdated !=''
      iswhere += " AND ls_fromdate >='#{year_month_days_formatted(search_fromdated)}' "
      session[:req_mysearch_fromdated]  = search_fromdated
      @search_fromdated = search_fromdated

     end
     if search_uptodated !=nil && search_uptodated !=''
      iswhere += " AND ls_todate <='#{year_month_days_formatted(search_uptodated)}' "
      session[:req_mysearch_uptodated] = search_uptodated
