...
class OutstandingadvanceamountdetailPdf < Prawn::Document
...
data5 = ([
  [{:content =>"S No."},{:content =>"Sewadar Code\n Old Code"},   {:content =>"Sewadar Name\nSewadar Department"}, {:content =>"Total Advance Amount"},   {:content =>"Total Installment"}, {:content =>"Current Outstanding"},{:content =>"No. Of Installment Left"},{:content =>"Last Installment Date"}],
  ...
  if @salary && @salary.length >0
    @salary.each do |newadv|
      if newadv.al_requesttype.to_s == 'Loan'
       ...
       if newadv.totalinstallment.to_f >0 && newadv.totaloutstanding.to_f >0
       totalinstalment = newadv.totalinstallment
       remainimonths   = newadv.totaloutstanding.to_f >0 ?  (newadv.totaloutstanding.to_f/totalinstalment.to_f) : 0
       leftinstalment  = remainimonths.to_f>0  ? remainimonths.to_i :  0 
       end
       leftinstdate    = newadv.leftinstallmentdate      
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newadv.al_sewadarcode+"\n"+newadv.sw_oldsewdarcode}, {:content =>newadv.sw_sewadar_name+"\n"+newadv.departname},  {:content =>currency_formatted(newadv.advancetotal).to_s}, {:content =>currency_formatted(newadv.totalinstallment).to_s}, {:content =>currency_formatted(newadv.totaloutstanding).to_s}, {:content =>leftinstalment.to_s}, {:content =>formatted_date(leftinstdate).to_s}]
