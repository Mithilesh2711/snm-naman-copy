...
class ZonedistrictPdf < Prawn::Document
  ...
  def line_items
          ...
          move_down 10
          text "<b><u>Zone District List</u></b>", :align => :center, :size =>10,:font_weight=>:bold,:inline_format=> TRUE
          Time.zone = "Kolkata"         
 ...
 def line_item_rows   
    [["SlNo.","DistrictCode","Zone","Name"]]+
    @invoice.each.map do |inv|
       [count,inv.zd_distcode.to_s,inv.myzones.to_s,inv.zd_name]
    end
end
