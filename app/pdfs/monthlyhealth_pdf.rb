...
class MonthlyhealthPdf < Prawn::Document
...
if @salary && @salary.length >0
         @salary.each do |newsalry|          
          nettotals     += newsalry.pm_ded_healthsewdarpay.to_f
          totals         = newsalry.pm_ded_healthsewdarpay.to_f
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsalry.pm_sewacode.to_s+"\n"+newsalry.sw_sewadar_name.to_s+"\n"+newsalry.deprtment.to_s},{:content =>currency_formatted(newsalry.pm_ded_healthslab).to_s},  {:content =>currency_formatted(newsalry.pm_ded_healthsewdarpay).to_s}, {:content =>currency_formatted(totals).to_s}]
