...
class BakshisdepartmentsummaryPdf < Prawn::Document
...
move_down 15
data2 = ([
  [{:content=>"Samagam Aashirwad - "+@chobj.bt_year.to_s},{:content=>"Print Date : "+format_oblig_date(Date.today).to_s}]
  ])
  ...
  totalothers    = 0
data4 = ([ 
  [{:content =>"Accounting Department "},{:content =>"Sum Of Samagam Aashirwad "+@chobj.bt_year.to_s}, {:content =>"BOI"}, {:content =>"PNB"}, {:content =>"Others (NEFT)"}],
  
  ])

    if @dataitem && @dataitem.length >0
      @dataitem.each do |newdept|
      totalaashirwad += currencyformatted_round(newdept.totalsangashd).to_f
      totalboi       += currencyformatted_round(newdept.totalboi).to_f
      totalpnb       += currencyformatted_round(newdept.totalpnb).to_f
      totalothers    += currencyformatted_round(newdept.othersneft).to_f

          data4 += ([
            [{:content =>newdept.bt_departname.to_s},{:content =>currencyformatted_round(newdept.totalsangashd).to_s},{:content =>currencyformatted_round(newdept.totalboi).to_s}, {:content =>currencyformatted_round(newdept.totalpnb).to_s}, {:content =>currencyformatted_round(newdept.othersneft).to_s}],
...
style row(1..5000),  :size => 8, :align=>:left
  style row(0..5000).column(0),   :width => 155
  style row(0..5000).column(1),   :width => 145
  style row(0..5000).column(2),   :width => 75
  style row(0..5000).column(3),   :width => 75
  style row(0..5000).column(4),   :width => 78
 
  style row(1..5000).column(0),  :size => 9, :align=>:left
  style row(1..5000).column(1..4),  :size => 9, :align=>:right
...
self.header= true
        ...
        data5 = ([
          [{:content =>"<b>Grand Total</b>",:inline_format=> true}, {:content =>"<b>"+currencyformatted_round(totalaashirwad).to_s+"</b>",:inline_format=> true}, {:content =>"<b>"+currencyformatted_round(totalboi).to_s+"</b>",:inline_format=> true}, {:content =>"<b>"+currencyformatted_round(totalpnb).to_s+"</b>",:inline_format=> true}, {:content =>"<b>"+currencyformatted_round(totalothers).to_s+"</b>",:inline_format=> true}],
...
table([] + data5,:width =>528)  do
  ...
  style row(0..5000).column(1..4),  :size => 9, :align=>:right
  style row(0..5000).column(0),   :width => 155
  style row(0..5000).column(1),   :width => 145
  style row(0..5000).column(2),   :width => 75
  style row(0..5000).column(3),   :width => 75
