...
class AdvancedetailPdf < Prawn::Document
...
data5 = ([
  [{:content =>"S No."},{:content =>"Request No.\nDate Of Request\nAdvance Type"},{:content =>"Sewadar Code\n Old Code\nName\nDepartment"},   {:content =>"Category\nMember Incharge"}, {:content =>"Date Of Birth\nDate Of Joining\nDate Of Superannuaton\nDate Of Relieving"},{:content =>"M.A Advance\n Advance Amount\n Installment"}, {:content =>"Sanction No.\n Sanction Date"}, {:content =>"Current Outstanding\nNo. Of Installments left\nLast Installment Date"}, {:content =>"Purpose\n Remark"}, {:content =>"Gurantor's Name"}, {:content =>"Status\nForwarded By(Sh./Smt.)"}],
...
if @salary && @salary.length >0
         @salary.each do |newadv| 
          approvestatus = ""         
          if newadv.al_approvestatus =='A'
        approvestatus = "Approved"
        elsif newadv.al_approvestatus =='R'
        approvestatus = " Rejected"
        elsif newadv.al_approvestatus =='H'
          approvestatus = "Hold"
        elsif newadv.al_approvestatus =='C'
          approvestatus = "Cancelled"
        else 
    
      if newadv.al_hod_status =='A' && newadv.al_approvestatus !='A'
    
        approvestatus =  "Forwarded To HR"
     else
...
if newadv.al_requesttype.to_s == 'Loan'
  ...
  totaladvan   = 0
  totalmadance = newadv.al_advanceamt.to_f+newadv.al_loanamount.to_f
else
  totaladvan   = newadv.al_advanceamt.to_f+newadv.al_loanamount.to_f
...
leftinstdate    = newadv.leftinstallmentdate
          ...
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newadv.al_requestno+"\n"+formatted_date(newadv.al_requestdate)+"\n"+requestype.to_s},{:content =>newadv.al_sewadarcode+"\n"+newadv.sw_oldsewdarcode+"\n"+newadv.sw_sewadar_name+"\n"+newadv.departname}, {:content =>newadv.categoryname+"\n"+newadv.memberincharge}, {:content =>formatted_date(newadv.sw_date_of_birth)+"\n"+formatted_date(newadv.sw_joiningdate)+"\n"+formatted_date(newadv.so_superannuationdate)+"\n"+formatted_date(newadv.so_regularizationdate)}, {:content =>currency_formatted(totalmadance).to_s+"\n"+currency_formatted(totaladvan).to_s+"\n"+currency_formatted(newadv.al_installpermonth).to_s}, {:content =>newadv.sanctionno+"\n"+formatted_date(newadv.sanctiondate)}, {:content =>currency_formatted(newadv.al_balances).to_s+"\n"+leftinstalment.to_s+"\n"+formatted_date(leftinstdate).to_s}, {:content =>newadv.al_purpose+"\n"+newadv.al_remark}, {:content =>newadv.al_guarantorname}, {:content =>approvestatus.to_s+"\n"+newadv.approvedby.to_s}]
...
style column(3), :width =>70
style column(4), :width =>80
style column(5), :width =>60
style column(6), :width =>70
style column(7), :width =>70
style column(8), :width =>70
style column(9), :width =>70
style column(10), :width =>80
