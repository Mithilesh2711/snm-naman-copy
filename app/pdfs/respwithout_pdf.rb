...
class RespwithoutPdf < Prawn::Document
...
data24 = ([
  [ {:content => ""},
    {:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj.sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" is doing sewa at "" "+@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", "+@compDetail.cmp_addressline1.to_s.to_s+". As per our records, his residential address is as under:"},
    {:content => ""}]
])
...
data25 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}"+personaladdress.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+"."},
    {:content => ""}]
])
table([] + data25, :width => 528) do
  style row(0), :border_width => 0 , :size => 11 , :align => :justify
...
data28 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}This certificate is issued at the request of "+@seawdarsobj.sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" as the same is required for notifying the residential address in Government documents."},
    {:content => ""}]
])
