...
netmaintenace =@seawdarsobj ? @seawdarsobj[0].so_totalgross : 0
@tnetamt =netmaintenace

move_down 30
data24 = ([
  [{:content => ""},
   {:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" has been rendering sewa at"" " +@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", " +@compDetail.cmp_addressline1.to_s+" since "+formatted_date(@seawdarsobj[0].sw_joiningdate).to_s+". As per our records, "+typeofme2.to_s+" is being paid Rs "+currency_formatted(netmaintenace).to_s+"/-  (Rupees "+number_currency_in_words+" Only) per month as Maintenance allowance."},
    {:content => ""}]
])
...
data25 = ([
  [{:content =>""},
   {:content => "#{Prawn::Text::NBSP*10}Sant Nirankari Mandal has no objection for "+typeofme.to_s+" visit to _____________. "},
   {:content => ""}]
])
...
data26 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}"+typeofme2.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" has also been provided with residential accommodation by the Sant Nirankari Mandal."},
    {:content => ""}]
])
...
data28 = ([
  [{:content => ""},
    {:content =>"This certificate is issued at the request of "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" for applying for a tourist VISA"},
    {:content => ""}]
])
