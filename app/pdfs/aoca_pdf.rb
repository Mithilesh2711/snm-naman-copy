...
data22 = ([
  [{:content => ""},
   {:content => "Ref: SNM/HRD/"+@seawdarsobj.sw_sewcode.to_s+"/"+dtyears.to_s+"/""______"},
...
data24 = ([
  [ {:content => ""},
    {:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj.sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" is doing sewa at"" "+@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", "+@compDetail.cmp_addressline1.to_s.to_s+" since "+formatted_date(@seawdarsobj.sw_joiningdate).to_s+" and has been provided with residential accommodation by the Sant Nirankari Mandal."},
    {:content => ""}]
])
...
data28 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}This certificate is issued at the request of "+@seawdarsobj.sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" as the same is required for completion of admission process of "+typeofme.to_s+" son/ daughter. "},
    {:content => ""}]
])
