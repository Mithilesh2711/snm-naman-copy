class GratuityreportPdf < Prawn::Document
  def initialize(exgratiadata,compdetail,uRl)
    super(:margin_top=>30,:bottom_margin=>30,:page_size =>"A4",:page_layout => :landscape )
    @exgratiadata = exgratiadata
...
style row(0).column(2), :align=>:right, :border_width => 0
 ...
 move_down 20
 text "Exgratia Register Report",:font_style => :bold, :size => 12
        move_up 12
        text "Print Date : "+format_oblig_date(Date.today).to_s, :size => 10, :align=>:right
            ...
            data4 = ([
              [{:content =>"S No."}, {:content =>"Sewadar Code"},   {:content =>"Name"},   {:content =>"Date Of Birth"},   {:content =>"Date Of Joining"}, {:content =>"Basic M.A"}, {:content =>"Gratuity Factor"}, {:content =>"No. of Year of Services as on Date \n (Current Date)" }, {:content =>"Gratuity Payable"}],
              
              ])
              if @exgratiadata && @exgratiadata.length >0
                @exgratiadata.each do |newpds|
                 
 
       data4 += ([
         [{:content =>count.to_s}, {:content =>newpds.pm_sewacode.to_s}, {:content =>newpds.sw_sewadar_name.to_s},   {:content =>formatted_date(newpds.sw_date_of_birth).to_s},   {:content =>formatted_date(newpds.sw_joiningdate).to_s},{:content =>currency_formatted(newpds.pm_basic).to_s}, {:content =>''}, {:content =>""}, {:content =>""}],
...
data6 = ([
 [{:content =>"Grand Total", :colspan => 3},  {:content =>""}, {:content =>""}, {:content =>""},{:content =>""},{:content =>''}, {:content =>""}],
