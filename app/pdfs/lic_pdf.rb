...
style column(1), :width => 385, :size => 12
...
data24 = ([
  [{:content => ""},
    {:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" is doing sewa at"" "+@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", "+@compDetail.cmp_addressline1.to_s+".",:inline_format => :true},
    {:content => ""}]
])
...
data25 = ([
  [{:content =>""},
   {:content => "#{Prawn::Text::NBSP*10}"+typeofme2.to_s+" is residing at "+personaladdress.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" in an accomodation provided by Sant Nirankari Mandal."},
   {:content => ""}]
])
...
data26 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}This certificate is issued at the request of "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" to enable "+typeofme3.to_s+" children to get higher education in Government Institutions."},
    {:content => ""}]
])
