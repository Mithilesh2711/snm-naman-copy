...
data24 = ([
  [{:content => ""},
  {:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" is doing sewa at"" "+@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", "+@compDetail.cmp_addressline1.to_s.to_s+" since "+formatted_date(@seawdarsobj[0].sw_joiningdate).to_s+" and "+typeofme2.to_s+" is being paid Maintenance Allowance of Rs."+currency_formatted(netmaintenace).to_s+"/- (Rupees "+number_currency_in_words+" Only) per month."},
    {:content => ""}]
])
...
data25 = ([
  [{:content =>""},
   {:content => "#{Prawn::Text::NBSP*10}"+typeofme2.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" has been provided with residential accommodation by Sant Nirankari Mandal at "+personaladdress.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+". The electricity is being provided to this accommodation from the common meter of Sant Nirankari Mandal."},
   {:content => ""}]
])
...
data28 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}This certificate is issued at the request of "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" as the same is required for applying for EWS Certificate."},
    {:content => ""}]
])
