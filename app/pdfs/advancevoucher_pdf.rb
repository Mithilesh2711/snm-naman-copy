...
class AdvancevoucherPdf < Prawn::Document
  ...
  def line_items
          ...
          move_down 10
          text "<b><u>ADVANCE LIST</u></b>", :align => :center, :size =>10,:font_weight=>:bold,:inline_format=> TRUE
          Time.zone = "Kolkata"         
 ...
 def line_item_rows   
    [["SlNo.","Sewdar Details","Request No\nRequest Date\nReques Type","Previous Balance","Installment","Total Sewa","Amount"]]+
    @invoice.each.map do |inv|
      newrefrcode = ""
      if inv.refercode !=nil && inv.refercode !=''
        newrefrcode = " ("+inv.refercode.to_s+")"
      end
       [count,inv.al_sewadarcode.to_s+newrefrcode.to_s+"\n"+inv.sewdarname.to_s+"\n"+inv.department.to_s+"\n"+inv.deginname.to_s,inv.al_requestno.to_s+"\n"+formatted_date(inv.al_requestdate).to_s+"\n"+inv.vd_requestfor.to_s,currency_formatted(inv.prevbalance).to_s,currency_formatted(inv.al_installpermonth).to_s,inv.totalsewa,currency_formatted(inv.aea_amount).to_s]
    end
end
