...
data24 = ([
  [{:content => ""},
  {:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" R/o "+personaladdress.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" has been rendering sewa at "" "+@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", "+@compDetail.cmp_addressline1.to_s.to_s+" since "+formatted_date(@seawdarsobj[0].sw_joiningdate).to_s+". As per our records, "+typeofme2.to_s+" is being paid Rs."+currency_formatted(netmaintenace).to_s+"/- (Rupees "+number_currency_in_words+" Only) per month as Maintenance Allowance."},
    {:content => ""}]
])
...
data28 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}This certificate is issued for the purpose of police verification as required for Guard Retainer of \n"+@compDetail.cmp_addressline1.to_s.to_s+"."},
    {:content => ""}]
])
