...
class BankPdf < Prawn::Document
  ...
  def line_items
          ...
          move_down 10
          text "<b><u>Bank List</u></b>", :align => :center, :size =>10,:font_weight=>:bold,:inline_format=> TRUE
          Time.zone = "Kolkata"         
 ...
 def line_item_rows   
    [["SlNo.","BankCode","BankName","IFSCCODE","Address"]]+
    @invoice.each.map do |inv|
       [count,inv.bl_code.to_s,inv.bl_name.to_s,inv.bl_ifsccode,inv.bl_address]
    end
end
