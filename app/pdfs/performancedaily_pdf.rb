class PerformancedailyPdf < Prawn::Document
  def initialize(attendance,compdetail,uRl,rsss,oths)
    super(:margin_top=>30,:bottom_margin=>30,:page_size => "A4" )
...
data4 = ([
  [{:content =>"S No."}, {:content =>"Employee Code\nName\nDate of Joining"},   {:content =>"Location\nDepartment"},   {:content =>"Shift"},{:content =>"Arrival Time"}, {:content =>"Dprt. Time"}, {:content =>"Working Hrs"}, {:content =>"Late Hrs"}, {:content =>"Early Hrs"}, {:content =>"OT"}, {:content =>"Remark"}],
  ...
  if @attendance && @attendance.length >0
    @attendance.each do |newpds|
      ...
      fwovtmns  += ovtmns.to_i
       ...
       data4 += ([
         [{:content =>count.to_s}, {:content =>newpds.al_empcode+"\n"+newpds.sw_sewadar_name.to_s+"\n"+formatted_date(newpds.sw_joiningdate).to_s},   {:content =>newpds.location.to_s+"\n"+newpds.mydepartment.to_s},   {:content =>newpds.al_shift.to_s},{:content =>newpds.al_arrtime.to_s}, {:content =>newpds.all_deptime.to_s}, {:content =>newpds.al_workhrs.to_s}, {:content =>newpds.al_latehrs.to_s}, {:content =>newpds.al_earlhrs.to_s}, {:content =>newpds.al_overtime.to_s}, {:content =>""}],
...
table([]+data4,:width =>528)  do
    ...
    style column(2), :width =>78
    style column(3), :width =>30
    style column(4), :width =>45
    style column(5), :width =>43
    style column(6), :width =>42
    style column(7), :width =>45
    style column(8), :width =>45
    style column(9), :width =>45
    style column(10), :width =>45
    #row(1).column(0).font_style = :bold
    row(0).column(0..11).font_style = :bold
    style row(1..5000).column(6..9),:align=>:right
...
data6 = ([
 [{:content =>""}, {:content =>"Grand Total"}, {:content =>"" }, {:content =>""}, {:content =>""}, {:content =>""}, {:content =>newwkhrs.to_s}, {:content =>newlthrs.to_s}, {:content =>newealhrs.to_s}, {:content =>newovthrs.to_s}, {:content =>""}],
