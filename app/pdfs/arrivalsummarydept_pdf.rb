class ArrivalsummarydeptPdf < Prawn::Document
  def initialize(attendance,compdetail,uRl,rsss,oths)
    super(:margin_top=>10,:bottom_margin=>30,:page_size => "A4" )
  ...
  move_down 40
   text "Department Wise Arrival Report"
          move_up 12
          text "Print Date : "+format_oblig_date(Date.today).to_s, :size => 10, :align=>:right            
          data4 = ([
            [{:content =>"Department"}, {:content =>"Total Strength"},   {:content =>"Present"},   {:content =>"Absent"},   {:content =>"On Leave"}, {:content =>"Present(%)"}],
       ...
       data4 += ([
         [{:content =>newpds.mydepartment.to_s}, {:content =>newpds.totalstrengthm.to_s}, {:content =>newpds.presents.to_s},   {:content =>newpds.absents.to_s},   {:content =>newpds.onleave.to_s}  ,{:content =>percentage.to_s}],
 ...
 table([] + data4,:width =>528)  do
  ...
  style row(0..5000).column(1..5), :align=>:center
  row(0).background_color = '87CEEB'
  # row(0..1).background_color = '153084'
  # row(0..1).text_color = "FFFFFF"
  style column(0), :width =>88
  style column(1), :width =>88
  style column(2), :width =>88
  style column(3), :width =>88
  style column(4), :width =>88
...
data6 = ([
 [{:content =>"Grand Total"},  {:content =>(tstrength.to_i).to_s}, {:content =>tpresents.to_s}, {:content =>tabsents.to_s},{:content =>tonleaves.to_s},{:content =>grandpercenatge.to_s}],
