...
class RespPdf < Prawn::Document
  def initialize(seawdarsobj,compdetail,uRl,sewadarpersonal,empchecked,empkyc,empkycbank,empqualif,familydetail,empworkexp,empstatelist,empdistrict,hodlisted,empdepartment)
    super(:left_margin=>35,:right_margin=>45,:top_margin=>5,:page_size =>"A4",:background => "public/assets/img/HRD1.png" )
...
data24 = ([
  [ {:content => ""},
    {:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj.sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" is doing sewa at "" "+@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", "+@compDetail.cmp_addressline1.to_s.to_s+". "+typeofme2.to_s+" has been provided residential accommodation by Sant Nirankari Mandal at "+personaladdress.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+"."},
    {:content => ""}]
])
...
data25 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}The electricity is being provided to this accommodation from the common meter of Sant Nirankari Mandal."},
    {:content => ""}]
])
table([] + data25, :width => 528) do
  style row(0), :border_width => 0 , :size => 11 , :align => :justify
...
data28 = ([
  [{:content => ""},
    {:content =>"#{Prawn::Text::NBSP*10}This certificate is issued at the request of "+@seawdarsobj.sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" as the same is required with notifying the residential address in Government documents."},
    {:content => ""}]
])
