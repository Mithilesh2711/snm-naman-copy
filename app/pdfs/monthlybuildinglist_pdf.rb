...
class MonthlybuildinglistPdf < Prawn::Document
 ...
 move_down 10
#  text "Building Deduction For the month of "+@salary[0].pm_paymonth.to_s+" "+@salary[0].pm_payyear.to_s
        move_up 12
        text "Date : "+format_oblig_date(Date.today).to_s, :size => 9, :align=> :right
data4 = ([
  [{:content =>"S No."}, {:content =>"Sewadar Code"},   {:content =>"Name\nDepartment"},{:content =>"Accomodation"}, {:content =>"Accomodation Type"}],
...
style column(4), :width =>118
style column(2..4), :align => :center
...
if @salary && @salary.length >0
         @salary.each do |newsalry|          
          ...
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsalry.sw_sewcode.to_s},{:content =>newsalry.sw_sewadar_name.to_s+"\n"+newsalry.departname.to_s},  {:content =>newsalry.sw_isaccommodation.to_s}, {:content =>newsalry.accomodationname.to_s}]
