...
class MemberlistPdf < Prawn::Document
  ...
  def line_items
          ...
          move_down 10
          text "<b><u>Member List</u></b>", :align => :center, :size =>10,:font_weight=>:bold,:inline_format=> TRUE
          Time.zone = "Kolkata"         
 ...
 def line_item_rows   
    [["SlNo.","MemberNo","Name","PanNo", "AdharNo", "PersonalEmail", "OfficialEmail", "MobileNo","DateOfBirth", "Address", "PinNo","Designation"]]+
    @invoice.each.map do |inv|
       [count,inv.lds_membno.to_s,inv.lds_name.to_s,inv.panno,inv.adharno,inv.email,inv.offemail,inv.mobileno,formatted_date(inv.lds_dob),inv.lds_address,inv.lds_pin,inv.mydesignation]
    end
end
