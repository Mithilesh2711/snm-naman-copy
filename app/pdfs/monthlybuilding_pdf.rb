...
class MonthlybuildingPdf < Prawn::Document
...
data4 = ([
  [{:content =>"S No."}, {:content =>"Sewadar Code\nName\nDepartment"},   {:content =>"Accomodation No"},{:content =>"Accomodation\n Amount"}, {:content =>"Total"}],
...
if @salary && @salary.length >0
         @salary.each do |newsalry|          
          nettotals     += newsalry.pm_dedaccomodatamount.to_f
          totals         = newsalry.pm_dedaccomodatamount.to_f
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsalry.pm_sewacode.to_s+"\n"+newsalry.sw_sewadar_name.to_s+"\n"+newsalry.deprtment.to_s},{:content =>newsalry.pm_dedaccomodationno.to_s},  {:content =>currency_formatted(newsalry.pm_dedaccomodatamount).to_s}, {:content =>currency_formatted(totals).to_s}]
