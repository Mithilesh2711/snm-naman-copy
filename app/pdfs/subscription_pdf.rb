...
class SubscriptionPdf < Prawn::Document
    ...
    def line_items
            ...
            move_down 10
            text "<b><u>Subscription</u></b>", :align => :center, :size =>10,:font_weight=>:bold,:inline_format=> TRUE
            Time.zone = "Kolkata"         
   ...
   def line_item_rows   
      [["Sl. No.", "Code", "Name", "Member", "Magazine", "SubscriptionType", "Currency", "ROE", "Quantity", "SubsAmount", "DispatchMode", "DispatchType", "DispatchTo"  "PaymentMode", "BankName", "DocumentNo.", "DocumentDate", "AmountReceived", "INRAmount", "SubsStartDate", "SubsEndDate", "Remarks", "ManualReceiptNo.", "ManualReceiptDate", "IssuingBranch", "Status"]]+
      @invoice.each.map do |inv|
         [count, inv.sub_code, inv.sub_name, inv.sub_member, inv.sub_magazine, inv.sub_subtyp, inv.sub_currency, inv.sub_roe, inv.sub_quantity, inv.sub_amount, inv.sub_dispatchmode, inv.sub_dispatchtype, inv.sub_dispatchto, inv.sub_paymentmode, inv.sub_bankname, inv.sub_docnum, inv.sub_docdate, inv.sub_amountrcv, inv.sub_inramount, inv.sub_startdate, inv.sub_enddate, inv.sub_remarks, inv.sub_receiptno, inv.sub_receiptdate, inv.sub_branch, inv.sub_status]
      end
  end
