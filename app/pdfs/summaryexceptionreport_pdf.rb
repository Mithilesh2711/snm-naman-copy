...
class SummaryexceptionreportPdf < Prawn::Document
...
data5 = ([
  [{:content =>"S No."},{:content =>"Sewadar Code\n Old Code"},   {:content =>"Sewadar Name\nSewadar Department"}, {:content=>"Category"},{:content =>"Superannuation Date"},{:content =>"Total Installment"}, {:content =>"Total Outstanding"}, {:content =>"Superannuation Balance Month"}],
  
  ])

if @salary && @salary.length >0
         @salary.each do |newsmry|          
          leftinstalment = 0
       
       if newsmry.totalinstallment.to_f >0 && newsmry.totaloutstanding.to_f >0
       totalinstalment = newsmry.totalinstallment
       remainimonths   = newsmry.totaloutstanding.to_f >0 ?  (newsmry.totaloutstanding.to_f/totalinstalment.to_f) : 0
       leftinstalment  = remainimonths.to_f>0  ? remainimonths.to_i :  0 
       end
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsmry.al_sewadarcode+"\n"+newsmry.sw_oldsewdarcode},{:content =>newsmry.sw_sewadar_name+"\n"+newsmry.departname}, {:content =>newsmry.categoryname},  {:content =>formatted_date(newsmry.so_superannuationdate)}, {:content =>newsmry.totalinstallment.to_i.to_s}, {:content =>newsmry.totaloutstanding.to_i.to_s},{:content =>leftinstalment.to_s}]
