...
class MonthlyelectricPdf < Prawn::Document
...
if @salary && @salary.length >0
         @salary.each do |newsalry|          
          nettotals     += newsalry.pm_ded_electricamount.to_f
          totals         = newsalry.pm_ded_electricamount.to_f
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsalry.pm_sewacode.to_s+"\n"+newsalry.sw_sewadar_name.to_s+"\n"+newsalry.deprtment.to_s},{:content =>newsalry.pm_ded_electricunit.to_s},  {:content =>currency_formatted(newsalry.pm_ded_electricamount).to_s}, {:content =>currency_formatted(totals).to_s}]
