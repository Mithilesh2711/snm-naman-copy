...
class LedgersPdf < Prawn::Document
  def initialize(report,compDetail,uRl,opbal,gprams,sewdar,sewacode,desiname,department,leavecodes,leavename,category)
    super(:margin_top=>20,:page_size =>"A4")
    @invoice    = report    
  ...
  def line_items
        ...
        move_down 2
          ...
          if @lastPmtDate!=nil && @lastPmtDate!=''
           ...
           text "<b>Ledger Register</b>", :align => :center, :size =>12,:inline_format=> TRUE
           
          if @from_date !=nil && @from_date !='' && @upto_date !='' && @upto_date !=nil
            text "<b>From Date : "+nfrdats.to_s+" Upto Date : "+nutdats.to_s+"</b>", :align => :center, :size =>7,:inline_format=> TRUE
          end
          ...
          if @category != nil && @category != ''
            text "Category : <b>"+@category.to_s+"</b>", :align => :center, :size =>8,:inline_format=> TRUE
          end
          if @department != nil && @department !=''
          text "Department : <b>"+@department.to_s+"</b>", :align => :center, :size =>8,:inline_format=> TRUE
          end
          if @desiname != nil && @desiname !=''
            text "Designation : <b>"+@desiname.to_s+"</b>", :align => :center, :size =>8,:inline_format=> TRUE
          end
 ...
 def line_item_rows
     obs = @opbal
	 balances    = obs
     [["Sl.No.","LeaveType","Debit","Credit","Balance"]]+
      @invoice.each.map do |inv|
		 ...
		 @clbal = balances
        [count,inv.leavecode.to_s,debitedleve.to_s,creiditedlv.to_s,balances.to_s]
      end
