...
class MonthlylicPdf < Prawn::Document
  ...
  def line_items
        ...
        data1 = ([
           [image_path,{:content => "<b>"+@compDetail.cmp_companyname.to_s+"</b>"+"\n"+@compDetail.cmp_typeofbussiness.to_s+",\n"+@compDetail.cmp_addressline2.to_s+",\n"+@compDetail.cmp_addressline3.to_s, :inline_format=> true , :size => 10}, {:content =>"Date : "+format_oblig_date(Date.today).to_s, :inline_format=> true, :size => 9}],
        ])
 ...
 move_down 10
 text "LIC Deduction For the month of "+@salary[0].pm_paymonth.to_s+" "+@salary[0].pm_payyear.to_s
data4 = ([
  [{:content =>"S No."}, {:content =>"Sewadar Code\nName\nDepartment"},   {:content =>"LIC Sewadar", :inline_format=> true },{:content =>"LIC Mandal"}, {:content =>"Total"}],
...
style row(0..4000).column(2..4),  :size => 10, :align=> :right
...
if @salary && @salary.length >0
         @salary.each do |newsalry|
          ...
          tlicemployer  += newsalry.pm_licemployer.to_f
          nettotals     += (newsalry.pm_ded_licemployee.to_f+newsalry.pm_licemployer.to_f).to_f
          totals        = newsalry.pm_ded_licemployee.to_f+newsalry.pm_licemployer.to_f
         
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsalry.pm_sewacode.to_s+"\n"+newsalry.sw_sewadar_name.to_s+"\n"+newsalry.deprtment.to_s},{:content =>currency_formatted(newsalry.pm_ded_licemployee).to_s},  {:content =>currency_formatted(newsalry.pm_licemployer).to_s}, {:content =>currency_formatted(totals).to_s}]
...
style row(0..4000).column(2..4), :align=> :right
end



data6 = ([
 [{:content =>"Grand Total"}, {:content =>""}, {:content =>currency_formatted(tlicamt).to_s }, {:content =>currency_formatted(tlicemployer).to_s}, {:content =>currency_formatted(nettotals).to_s}],
