...
class MonthlyelectriclistPdf < Prawn::Document
 ...
 move_down 10
#  text "Electric Deduction For the month of "+@salary[0].pm_paymonth.to_s+" "+@salary[0].pm_payyear.to_s
#         move_up 12
        text "Print Date : "+format_oblig_date(Date.today).to_s, :size => 10, :align=>:right
data4 = ([
  [{:content =>"S No."}, {:content =>"Sewadar Code"},   {:content =>"Name\nDepartment"},{:content =>"Electric Consumption "}, {:content =>"Meter No "}],
...
style column(2), :width =>220
style column(3), :width =>80
style column(4), :width =>98
...
if @salary && @salary.length >0
         @salary.each do |newsalry|          
          ...
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsalry.sw_sewcode.to_s},{:content =>newsalry.sw_sewadar_name.to_s+"\n"+newsalry.departname.to_s},  {:content =>newsalry.sw_iselectricconsump.to_s}, {:content =>newsalry.sw_meterno.to_s}]
