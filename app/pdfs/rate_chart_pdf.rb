...
class RateChartPdf < Prawn::Document
    ...
    def line_items
            ...
            move_down 10
            text "<b><u>RateChart</u></b>", :align => :center, :size =>10,:font_weight=>:bold,:inline_format=> TRUE
            Time.zone = "Kolkata"         
   ...
   def line_item_rows   
      [["SlNo.","Code","Name","Currency","SubscriptionType","Amount","Magazine","Status"]]+
      @invoice.each.map do |inv|
         [count,inv.rc_code,inv.rc_name,inv.rc_currency,inv.rc_subtyp,inv.rc_amount, inv.rc_magazine, inv.rc_status]
      end
  end
