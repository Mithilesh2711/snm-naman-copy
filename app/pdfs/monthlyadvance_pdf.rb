...
class MonthlyadvancePdf < Prawn::Document
 ...
 move_down 10
 text " Advance Deduction For the month of "+@salary[0].pm_paymonth.to_s+" "+@salary[0].pm_payyear.to_s
        move_up 12
        text "Date : "+format_oblig_date(Date.today).to_s, :size => 9,:align=>:right
data4 = ([
  [{:content =>"S No."}, {:content =>"Sewadar Code\nReference Code"},   {:content =>"Name"},{:content =>"Department"}, {:content =>"Advance\n Amount"}],
...
if @salary && @salary.length >0
         @salary.each do |newsalry|          
          nettotals     += (newsalry.pm_ded_repaidadvance.to_f+newsalry.pm_ded_repaidloan.to_f).to_f
          totals        = newsalry.pm_ded_repaidadvance.to_f+newsalry.pm_ded_repaidloan.to_f
          advamt        += newsalry.pm_ded_repaidadvance.to_f
          loanamt       += newsalry.pm_ded_repaidloan.to_f
          data5 += ([
            [{:content =>mycount.to_s}, {:content =>newsalry.pm_sewacode.to_s+"\n "+newsalry.sw_oldsewdarcode.to_s},{:content =>newsalry.sw_sewadar_name.to_s},  {:content =>newsalry.deprtment.to_s}, {:content =>currency_formatted(totals).to_s}]
...
data6 = ([
 [{:content =>"Grand Total",:colspan=>4}, {:content =>currency_formatted(nettotals).to_s}],
