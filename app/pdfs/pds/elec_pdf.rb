...
move_down 30
data24 = ([
  [{:content => "This is to certify that "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" "+typeofx.to_s+" "+namex.to_s+" has been rendering sewa at "+@compDetail.cmp_addressline1.to_s.to_s+" since "+@seawdarsobj.sw_joiningdate.to_s+". As per our records, "+typeofme2.to_s+" is being paid Rs 36,713/- per month(Rupees thirty six thousand seven hundred and thirteen only) as Maintenance allowance. "+typeofme2.to_s+" has applied for 15 day's leaves from 25th March to 8th April 2022."},
    {:content => ""},
    {:content => ""}]
])
...
data25 = ([
  [{:content =>""},
   {:content => "Sant Nirankari Mandal has no objection for "+typeofme.to_s+" visit to Switzerland. "},
   {:content => ""}]
])
...
data28 = ([
  [{:content => ""},
    {:content =>"This certificate is issued at the request of "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" for applying for a tourist VISA"},
    {:content => ""}]
])
