...
table([] + data23, :width => 528) do
  ...
  style row(0).column(1), :font_style => :bold, :border_bottom_width => 1 , :width => 230
end
...
move_down 30
data24 = ([
  [{:content => "This is to certify that "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" "+typeofx.to_s+" "+namex.to_s+" is doing sewa at "+@compDetail.cmp_addressline1.to_s.to_s+". "+typeofme2.to_s+" is being paid total Maintenance Allowance w.e.f. June, 2021 as under:-"},
    {:content => ""},
    {:content => ""}]
])
...
data26 = ([
  [{:content => "Month"}, {:content => "Gross M.A. per month"},{:content => "Deductions", :colspan => 3},{:content => "Total Deductions"},{:content => "M.A. Paid After Deductions per month"}],
    [{:content =>""},{:content =>""},{:content =>"Health Insurance "},{:content =>"Advance"},{:content =>"Leave without M.A."},{:content =>""},{:content =>""}],
    [{:content => "June, 2021"},{:content => "Rs.20,286/-"},{:content => "Rs.1,580/-"},{:content => "Rs.1000/-"},{:content => "Rs.676/-"},{:content => "Rs.3,256/-"},{:content => "Rs.17,030/-"}]
])
table([] + data26, :width => 522) do
  style row(0..1).column(0), :size => 11 , :align => :center , :width => 70 , :font_style => :bold
  style row(0..1).column(1), :size => 11 , :align => :center , :width => 70, :font_style => :bold
  style row(0..1).column(2), :size => 11 , :align => :center  , :width => 80, :font_style => :bold
  style row(0..1).column(3), :size => 11 , :align => :center , :width => 70, :font_style => :bold
  style row(0..1).column(4), :size => 11 , :align => :center , :width => 80, :font_style => :bold
  style row(0..1).column(5), :size => 11 , :align => :center , :width => 70, :font_style => :bold
  style row(0..1).column(6), :size => 11 , :align => :center , :width => 82, :font_style => :bold
...
data28 = ([
  [{:content => ""},
    {:content =>"This certificate is issued at the request of "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+", Sewadar for the purpose of securing Bank loan."},
    {:content => ""}]
])
