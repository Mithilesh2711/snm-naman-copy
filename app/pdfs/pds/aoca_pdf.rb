...
move_down 30
data24 = ([
  [{:content => "This is to certify that "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" "+typeofx.to_s+" "+namex.to_s+" is doing sewa at "+@compDetail.cmp_addressline1.to_s.to_s+" since "+@seawdarsobj.sw_joiningdate.to_s+" and has been provided with residential accommodation by the Sant Nirankari Mandal.
    "},
    {:content => ""},
    {:content => ""}]
])
...
data28 = ([
  [{:content => ""},
    {:content =>"This certificate is issued at the request of "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" as the same is required for completion of admission process of "+typeofme.to_s+" son. "},
    {:content => ""}]
])
