...
move_down 30
data24 = ([
  [{:content => "	This is to certify that "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" "+typeofx.to_s+" "+namex.to_s+" is doing sewa at "+@compDetail.cmp_addressline1.to_s.to_s+". "+typeofme2.to_s+" has been provided residential accommodation by Sant Nirankari Mandal at ____________________________. "},
    {:content => ""},
    {:content => ""}]
])
...
data28 = ([
  [{:content => ""},
    {:content =>"This certificate is issued at the request of "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" as the same is required for notifying the new residential address in the Government documents."},
    {:content => ""}]
])
