...
move_down 30
data24 = ([
  [{:content => "This is to certify that "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" "+typeofx.to_s+" "+namex.to_s+" is doing sewa at "+@compDetail.cmp_addressline1.to_s.to_s+" since "+@seawdarsobj.sw_joiningdate.to_s+" and "+typeofme2.to_s+"  is being paid monthly Maintenance Allowance of Rs.28,472/- per month (Rupees Twenty eight thousand four hundred and seventy two only). 
    "},
    {:content => ""},
    {:content => ""}]
])
...
data25 = ([
  [{:content =>""},
   {:content => ""+typeofme2.to_s+"  has been provided with residential accommodation by Sant Nirankari Mandal at __________________________________________. The electricity is being provided to this accommodation from the common meter of Sant Nirankari Mandal.
...
data28 = ([
  [{:content => ""},
    {:content =>"This certificate is issued at the request of "+typeofx2.to_s+" "+(@seawdarsobj ? @seawdarsobj.sw_sewadar_name : '' ).to_s+" as the same is required for applying for EWS Certificate.
      "},
    {:content => ""}]
])
