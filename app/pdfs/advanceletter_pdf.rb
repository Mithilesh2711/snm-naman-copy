...
class AdvanceletterPdf < Prawn::Document
...
data22 = ([
  [{:content => "Subject:<u> Grant of advance of Maintenance Allowance to "+sewaprefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewdarname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ") : '' ).to_s+", "+departmentname.to_s.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" </u>"}],
   [{:content => ""}],
])
...
amounts       = @seawdarsobj ? @seawdarsobj[0].totalamount : 0
installment   = @seawdarsobj ? @seawdarsobj[0].al_installpermonth : 0
...
noofinstallment = ""
if amounts.to_f >0
  noofinstallment = amounts.to_f/installment.to_f
end

move_up 18
data23 = ([
  [{:content => "  (Rupees "+number_currency_in_words+" Only ) to " +"<b>"+sewaprefix.to_s+". "+sewdarname.to_s.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+"</b>"+", "+"<b>"+"("+@seawdarsobj[0].sw_sewcode.to_s+")"+"</b>"+", "+"<b>"+departmentname.to_s.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+"</b>"+" on his request no."+@seawdarsobj[0].al_requestno.to_s+"." + " The advance is recoverable in "+(noofinstallment.to_i).to_s+" instalment of Rs. "+installment.to_f.round(0).to_s+"/- per month. The last instalment will be Rs. "+installment.to_f.round(0).to_s+"/-."}],
   [{:content => ""}],
])
...
data23 = ([
  [{:content=> ""},{:content => "The payment may be made to "+"<u><b>"+sewaprefix.to_s+". "+sewdarname.to_s.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+"</b></u>"+" by account payee cheque.   "}],
   [{:content => ""}],
])
