...
class ItrPdf < Prawn::Document
  def initialize(seawdarsobj,compdetail,uRl,sewadarpersonal,empchecked,empkyc,empkycbank,empqualif,familydetail,empworkexp,empstatelist,empdistrict,hodlisted,empdepartment,gsess)
...
data22 = ([
  [{:content => ""},
   {:content => "SNM/HRD/"+@seawdarsobj[0].sw_sewcode.to_s+"/"+dtyears.to_s+"/""______"},
...
move_down 20
data24 = ([
  [{:content => "#{Prawn::Text::NBSP*10}This is to certify that "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+typeofx.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" "+namex.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" is doing sewa at"" " +@compDetail.cmp_companyname.to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+", "+@compDetail.cmp_addressline1.to_s+". "+typeofme2.to_s+" has been paid Maintenance Allowance etc. during the financial year " +@gsess["requestyear"].to_s+ " as under:-"},
    {:content => ""},
    {:content => ""}]
])
...
data26 = ([
  [{:content => "S.No."}, {:content => "Name of Allowance"},{:content => " "},{:content => "Amount"}],
    [{:content =>"1."},{:content =>"Maintenance Allowance"},{:content =>" "},{:content =>"Rs."++""}],
    [{:content =>"2."},{:content =>"SamagamAshirwad"},{:content =>" "},{:content =>"+Rs."++""}],
    [{:content =>"3."},{:content =>"Education Aid"},{:content =>" "},{:content =>"+Rs."}],
    [{:content => " "},{:content => " "},{:content => "Total"},{:content => "=Rs."+""+""}]
])
table([] + data26, :width => 523) do
  style row(0), :size => 11 , :align => :left ,  :font_style => :bold , :border_width => 0
  style row(1), :size => 11 , :align => :left , :border_width => 0
  style row(2), :size => 11 , :align => :left , :border_width => 0
  style row(3), :size => 11 , :align => :left , :border_width => 0
  style row(4), :size => 11 , :align => :left ,  :font_style => :bold , :border_left_width => 0, :border_right_width => 0,:border_bottom_width=>0
  style row(0..4).column(0), :size => 11 , :align => :center , :border_width => 0 , :width => 70
  style row(0..4).column(1), :size => 11 , :align => :left , :border_width => 0 , :width => 189
  style row(0..4).column(2), :size => 11 , :align => :left , :border_width => 0 , :width => 112
  style row(0..4).column(3), :size => 11 , :align => :left , :border_width => 0 , :width => 152
cells.padding = 3
...
data261 = ([
  [{:content => "S.No."}, {:content => "Deductions:-"},{:content => " "},{:content => "Amount"}],
    [{:content =>"1."},{:content =>"Building Maintenance"},{:content =>" "},{:content =>"=(-)Rs."++""}],
    [{:content =>"2."},{:content =>"Deduction on account of \n Premium of Group Saving Linked Insurance Scheme",:colspan=>2},{:content =>"\n=(-)Rs."++""}],
    [{:content => "3. "},{:content => "Premium of Health Insurance "},{:content => ""},{:content => "=(-)Rs."++""}],
    [{:content => " "},{:content => " "},{:content => "Total Deductions"},{:content => "= Rs."+""}]
])
table([] + data261, :width => 523) do
  style row(0), :size => 11 , :align => :left ,  :font_style => :bold , :border_width => 0
  style row(1), :size => 11 , :align => :left , :border_width => 0
  style row(2), :size => 11 , :align => :left ,  :border_width => 0
  style row(3), :size => 11 , :align => :left ,  :border_width => 0
  style row(4), :size => 11 , :align => :left ,  :font_style => :bold , :border_left_width => 0, :border_right_width => 0
  style row(0..3).column(0), :size => 11 , :align => :center , :border_width => 0 , :width => 70
  style row(0..3).column(1), :size => 11 , :align => :left , :border_width => 0 , :width => 189
  style row(0..3).column(2), :size => 11 , :align => :left , :border_width => 0 , :width => 112
  style row(0..3).column(3), :size => 11 , :align => :left , :border_width => 0 , :width => 152
  cells.padding = 3
style row(4).column(0),:border_width=> 0
...
data263 = ([
    ...
    [{:content => " "},{:content => " "},{:content => "Amount Paid"},{:content => "= Rs."+""+""}]
])
table([] + data263, :width => 523) do
 



  style row(0), :size => 11 , :align => :left ,  :font_style => :bold , :border_left_width => 0, :border_right_width => 0
  style row(0..3).column(0), :size => 11 , :align => :left , :font_style => :bold , :border_left_width => 0, :border_right_width => 0 , :width => 70
  style row(0..3).column(1), :size => 11 , :align => :left ,:font_style => :bold , :border_left_width => 0, :border_right_width => 0 , :width => 189
  style row(0..3).column(2), :size => 11 , :align => :left , :font_style => :bold , :border_left_width => 0, :border_right_width => 0, :width => 112
  style row(0..3).column(3), :size => 11 , :align => :left , :font_style => :bold , :border_left_width => 0, :border_right_width => 0, :width => 152
  cells.padding = 3
style row(0).column(0),:border_width=> 0
...
# text "(Rupees Three lac forty seven thousand five hundred and two only)" , :align=>:center 

move_down 5
accomodation = @seawdarsobj[0] ? @seawdarsobj[0].accomodation  : ''
if accomodation.to_s== 'Y'
data26 = ([
  [{:content => ""},
    {:content =>""+typeofme2.to_s+" has also been provided with residential accommodation by the Sant Nirankari Mandal"},
    {:content => ""}]
])
table([] + data26, :width => 528) do
  style row(0), :border_width => 0 , :size => 11 , :align => :justify
...
data28 = ([
  [{:content => ""},
    {:content =>"This certificate is issued at the request of "+@seawdarsobj[0].sw_sewadar_prefix.to_s+". "+(@seawdarsobj ? @seawdarsobj[0].sewadarname : '' ).to_s.split(/ |\_|\-/).map(&:capitalize).join(" ")+" for filing Income Tax Return."},
    {:content => ""}]
])
