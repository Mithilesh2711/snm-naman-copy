...
class ProductsPdf < Prawn::Document
  ...
  def line_items
          ...
          move_down 10
          text "<b><u>Product List</u></b>", :align => :center, :size =>10,:font_weight=>:bold,:inline_format=> TRUE
          Time.zone = "Kolkata"         
 ...
 def line_item_rows   
    [["SlNo.","ProductCode","ProductName","Opening Balance","Closing Balance"]]+
    @invoice.each.map do |inv|
       [count,inv.pr_code.to_s,inv.pr_name.to_s,inv.pr_ob,inv.pr_cb]
    end
end
