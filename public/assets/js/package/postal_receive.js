...
function filter_dispatched(){
    ...
    var datefirst = $.trim( $("#search_fromdated").val() );
    var datesec   = $.trim( $("#search_uptodated").val() );
   
    if ( datefirst !=''  &&  datesec !=''){
        var date1 = new Date(datefirst);
        var date2 = new Date(datesec);
        if( date1 >date2){
            $("#search_fromdated").focus();
            alert("Upto Date should be greater than from date");
            return false;
        }
    }
    $("form#myforms").attr("action",useroot+"postal_receive/search");   
    $("form#myforms").submit();
}
