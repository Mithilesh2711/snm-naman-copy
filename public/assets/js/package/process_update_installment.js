...
function get_installment_amount(){
    ...
    $.ajax({
                 url: usePath+"process_update_installment/ajax_process",
                 type: 'POST',
                 data: {'empcode': empcode,'requestno':requestno,'identity':'REQAMT'},
                 async: false,
                 success: function (resp) {
                   var sdata = resp.data;                     
                   if( resp.status){
                       $("#trns_old").val(sdata.al_installpermonth);
                   }else{
                    $("#trns_old").val(''); 
                   }
...
function filter_installment(){
    ...
    $("form#myforms").attr("action",useroot+"process_update_installment/search");   
    $("form#myforms").submit();
}
